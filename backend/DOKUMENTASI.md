...
- **Database:** SQLite in-memory (di-override dari PostgreSQL)
- **Trait:** `DatabaseTransactions` (auto-rollback per test)

### 7.2 Test yang Ada (`tests/Feature/AuthTest.php`)

| Test | Deskripsi |
|------|-----------|
| `test_register_success` | Register berhasil → 201 + token |
| `test_register_duplicate_email_fails` | Email duplikat → 422 |
| `test_login_success` | Login berhasil → 200 + token |
| `test_login_wrong_password_fails` | Password salah → 401 |
| `test_forgot_password_unregistered_email_returns_success` | Email tidak terdaftar → tetap 200 |
